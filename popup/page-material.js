          ...
          <div class="card_content">
            <div style="color: #888;">
              ...
              <div style="margin-bottom: 4px;">
                <div>当日词频：</div>
                <div>
                  ${bluesea.forStatisticTodayCount(
                    material,
                  )}
                </div>
              </div>
...
const ModalLearnCard = ({
  ...
  const needLearn =
    dayjs().format() > material.learn.learnDate;
...
const ModalSortRules = ({ visible, onClose: close }) => {
  ...
  return html`
    ...
    >
      ...
      >
        ${Object.entries(bluesea.constant.sortRule).map(
          ([key, label]) => {
...
const Material = () => {
  const { list, sortRule, setSortRule } =
    bluesea.useMaterialsMate();
  let config = bluesea.useConfig();

  const [selectedMaterial, setSelectedMaterial] =
    useState(null);
  const [visibleModalSortRules, setVisibleModalSortRules] =
    useState(false);
  const [visibleModalExport, setVisibleModalExport] =
    useState(false);
    ...
    >
      ...
      >
        ...
        >
          ...
          <div style="color: #888;margin-right: 12px;">
            排序数据
          </div>
        ...
        ${list.map((it, index) => {
          ...
          if (nowDate < it.learn.learnDate) {
            ...
            const diffMinute =
              dayjs().diff(it.learn.learnDate, "minute") ||
              1;
          ...
          >
            <div style="flex: 1;display: flex;">
              ...
              <div
                style="margin-left:16px; font-weight:300; cursor:default;"
              >
                ${it.translation}
              </div>
              <div style="flex:1;"></div>
